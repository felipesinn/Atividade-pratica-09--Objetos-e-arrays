...
function criarPessoa() {
    ...
    const trabalhando = prompt("A pessoa está trabalhando? (Sim/Não)").toLowerCase() === "sim";
...
do {
    ...
    const continuar = prompt("Deseja continuar cadastrando? (Sim/Não)").toLowerCase() === "sim";
    if (!continuar) {
        break;
    }
} while (true);
...
const menores2500 = pessoas.filter(pessoa => pessoa.trabalhando && pessoa.salario < 2500);
const maiores2500 = pessoas.filter(pessoa => pessoa.trabalhando && pessoa.salario >= 2500);
